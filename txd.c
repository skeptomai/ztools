...
#else
static int decode_parameters(opers) int *opers;
#endif
{
...
#else
static int decode_parameter(addr_mode, opers) int addr_mode;
int opers;
#endif
{
	unsigned long addr;
...
#else
static int lookup_string(addr) unsigned long addr;
#endif
{
	cref_item_t *cref_item;
...
#else
static int in_dictionary(word_address) unsigned long word_address;
#endif
{
...
#else
static int lookup_label(addr, flag) unsigned long addr;
int flag;
#endif
{
...
#else
static int lookup_routine(addr, flag) unsigned long addr;
int flag;
#endif
{
...
#else
static int print_object_desc(obj) unsigned int obj;
#endif
{
...
#else
static void dump_operand(addr, addr_mode, opers, par, count) unsigned long *addr;
int addr_mode;
int opers;
int *par;
int *count;
...
#endif
{
	...
	else if (option_symbols && print_global_name(start_of_routine, varnum - 16)) /* null */
		;
	else
